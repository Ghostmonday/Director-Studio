...
final class SegmentingModule: SegmentingModuleProtocol {
    ...
    func segment(
        ...
        print("📝 Script: \(script.count) characters")
        print("🎯 Mode: \(mode.displayName)")
        print("⚙️  Constraints: max \(constraints.maxSegments) segments, \(constraints.maxTokensPerSegment) tokens each")
        #endif
        ...
        #if DEBUG
        print("✅ [SegmentingModule] Complete")
        print("📊 Segments: \(segments.count)")
        print("🎯 Avg Confidence: \(Int(avgConfidence * 100))%")
        print("⏱️  Time: \(String(format: "%.2f", executionTime))s")
        if !warnings.isEmpty {
...
final class SemanticExpansionProcessor {
    ...
    /// Public method for testing - identifies which segments should be expanded
    func identifyExpansionCandidates(
        _ segments: [CinematicSegment],
        config: SemanticExpansionConfig
    ) -> [Int] {
        return identifyExpansionCandidates(segments, config: config)
    }
    
    /// Public method for testing - detects emotional intensity
    func detectEmotionalIntensity(_ text: String) -> Double {
        return detectEmotionalIntensity(text)
    }
