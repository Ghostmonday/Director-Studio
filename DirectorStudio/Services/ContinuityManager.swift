...
final class ContinuityManager: ContinuityManagerProtocol, @unchecked Sendable {
    ...
    func getContinuitySummary() async -> String {
        return sceneContext.summary()
    }
    
    /// Updates continuity based on user feedback
    func updateContinuityElement(element: ContinuityElement, value: String) async {
        switch element {
        case .character:
