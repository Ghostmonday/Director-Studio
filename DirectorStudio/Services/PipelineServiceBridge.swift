...
class PipelineServiceBridge {
    ...
    private let generationQueue = GenerationQueue.shared
    ...
    func generateClip(
        ...
        if enabledStages.contains(.continuityAnalysis) {
            ...
            continuityAnalysis = await continuityManager.analyzeContinuity(
                prompt: prompt,
        ...
        if enabledStages.contains(.continuityInjection), let analysis = continuityAnalysis {
            ...
            processedPrompt = await continuityManager.injectContinuity(
                prompt: prompt,
                analysis: analysis,
                referenceImage: referenceImageData
            )
        ...
        if let imageData = referenceImageData {
            ...
            // Queue video generation through GenerationQueue
            let request = GenerationRequest(prompt: enhancedPrompt) {
                // If using KlingAIService, pass tier and camera control
                if let klingService = serviceToUse as? KlingAIService {
                    return try await klingService.generateVideoFromImage(
                        imageData: imageData,
                    ...
                    return try await serviceToUse.generateVideoFromImage(
                        imageData: imageData,
                        prompt: enhancedPrompt,
                        duration: duration
                    )
            ...
            videoURL = try await generationQueue.enqueue(request)
            ...
            let request = GenerationRequest(prompt: enhancedPrompt) {
                // If using KlingAIService, pass tier and camera control
                if let klingService = serviceToUse as? KlingAIService {
                    return try await klingService.generateVideo(
                        prompt: enhancedPrompt,
                        duration: duration,
                        tier: tier,
                        cameraControl: cameraControl
                    ...
                    return try await serviceToUse.generateVideo(
                        prompt: enhancedPrompt,
                        duration: duration
                    )
                }
