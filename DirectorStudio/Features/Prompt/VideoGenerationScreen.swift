...
class FilmGeneratorViewModel: ObservableObject {
    ...
    /// Generation queue for managing concurrent video generations
    ...
    private func generateTake(take: FilmTake, takeIndex: Int, prompt: String, tier: VideoQualityTier = .basic) async throws -> GeneratedClip {
        ...
        if takeIndex == 0 {
            ...
            let request = GenerationRequest(prompt: videoPrompt) {
                return try await self.klingVideoService.generateVideo(
                    prompt: videoPrompt,
            ...
            generatedVideoURL = try await generationQueue.enqueue(request)
        } else {
            ...
            // Queue video generation through GenerationQueue
            let request = GenerationRequest(prompt: videoPrompt) {
                return try await self.klingVideoService.generateVideoFromImage(
